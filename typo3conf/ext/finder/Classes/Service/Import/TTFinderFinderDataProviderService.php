...
/**
 * tt_finder ImportService
 *
 * @package TYPO3
 * @subpackage tx_finder
 * @author Nikolas Hagelstein <nikolas.hagelstein@gmail.com>
 */
class Tx_Finder_Service_Import_TTFinderFinderDataProviderService implements Tx_Finder_Service_Import_DataProviderServiceInterface, t3lib_Singleton {
	...
	public function getImportData($offset = 0, $limit = 50) {
		...
		while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
			$importData[] = array(
				...
				'content_elements' => $row['tx_rgfinderce_ce'],
				'import_id' => $row['uid'],
	...
	protected function getFiles(array $row) {
		if (empty($row['finder_files'])) {
			return FALSE;
		}

		$relatedFiles = array();

		$files = t3lib_div::trimExplode(',', $row['finder_files']);
	...
	/**
	 * Get correct categories of a finder record
	 *
	 * @param integer $finderUid finder uid
	 * @return array
	 */
	protected function getCategories($finderUid) {
		$categories = array();
	 ...
	 *
	 * @param string $finderLinks
	 * @return array
	 */
	protected function getRelatedLinks($finderLinks) {
		$links = array();

		if (empty($finderLinks)) {
			return $links;
		}

		$finderLinks = str_replace(array('<link ', '</link>'), array('<LINK ', '</LINK>'), $finderLinks);

		$linkList = t3lib_div::trimExplode('</LINK>', $finderLinks, TRUE);
