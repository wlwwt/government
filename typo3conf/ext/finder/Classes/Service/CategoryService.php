 ...
 */
class Tx_Finder_Service_CategoryService {
	...
	private static function getChildrenCategoriesRecursive($idList, $counter, $additionalWhere) {
		...
		$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
			...
			'tx_finder_domain_model_category.parentcategory IN (' . $GLOBALS['TYPO3_DB']->cleanIntList($idList) . ')
				AND deleted=0 ' . $additionalWhere);

		while (($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res))) {
			$counter++;
			if ($counter > 10000) {
				$GLOBALS['TT']->setTSlogMessage('EXT:finder: one or more recursive categories where found');
