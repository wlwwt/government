...
/**
 * Controller to import finder records from tt_finder
 *
 * @package TYPO3
 * @subpackage tx_finder
 */
class Tx_Finder_Controller_ImportController extends Tx_Extbase_MVC_Controller_ActionController {
	...
	protected function getAvailableJobs() {
		$availableJobs = array();
		$registeredJobs = Tx_Finder_Utility_ImportJob::getRegisteredJobs();

		foreach ($registeredJobs as $registeredJob) {
			$jobInstance = $this->objectManager->get($registeredJob['className']);
			if ($jobInstance instanceof Tx_Finder_Jobs_ImportJobInterface && $jobInstance->isEnabled()) {
