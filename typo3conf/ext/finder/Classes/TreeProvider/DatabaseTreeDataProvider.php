 ...
 */
class Tx_Finder_TreeProvider_DatabaseTreeDataProvider extends t3lib_tree_Tca_DatabaseTreeDataProvider {
	...
	protected function isCategoryAllowed ($child) {
		$mounts = Tx_Finder_Utility_CategoryProvider::getUserMounts ();
		if (empty($mounts)) {
			return TRUE;
		}
	...
	/**
	 * By setting "tx_finder.singleCategoryAcl = 1" in UserTsConfig
	 * every category needs to be activated, no recursive enabling
	 *
	 * @return bool
	 */
	protected function isSingleCategoryAclActivated() {
		if (is_array($GLOBALS['BE_USER']->userTS['tx_finder.'])
			&& $GLOBALS['BE_USER']->userTS['tx_finder.']['singleCategoryAcl'] === '1'
		) {
			return TRUE;
		}
