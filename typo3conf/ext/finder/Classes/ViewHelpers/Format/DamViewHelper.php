 ...
 */
class Tx_Finder_ViewHelpers_Format_DamViewHelper extends Tx_Fluid_Core_ViewHelper_AbstractViewHelper {
	...
	public function render($uid, $as) {
		...
		if ($GLOBALS['TSFE']->sys_language_content > 0) {
			$media = $GLOBALS['TYPO3_DB']->exec_SELECTgetSingleRow('*', 'tx_finder_domain_model_media', 'deleted=0 AND uid =' . $uid);
			$media = $GLOBALS['TSFE']->sys_page->getRecordOverlay('tx_finder_domain_model_media', $media, $GLOBALS['TSFE']->sys_language_content);
			if ($media['_LOCALIZED_UID'] > 0) {
				// Does this localized media has dam record?
				$where = 'uid_foreign =' . (int)$media['_LOCALIZED_UID'] . ' AND tablenames =\'tx_finder_domain_model_media\' AND ident = \'tx_finder_media\'';
