 ...
 */
class Tx_Finder_Utility_CategoryProvider {
	...
	public static function getUserMounts() {
		...
		if (is_array($GLOBALS['BE_USER']->userGroups)) {
			foreach ($GLOBALS['BE_USER']->userGroups as $group) {
				if ($group['tx_finder_categorymounts']) {
					$categoryMounts .=  ',' . $group['tx_finder_categorymounts'];
				}
			}
		}

		// Mounts of the user record
		if ($GLOBALS['BE_USER']->user['tx_finder_categorymounts']) {
			$categoryMounts .= ',' . $GLOBALS['BE_USER']->user['tx_finder_categorymounts'];
		}
