...
/**
 * Finder repository with all the callable functionality
 *
 * @package TYPO3
 * @subpackage tx_finder
 */
class Tx_Finder_Domain_Repository_FinderRepository extends Tx_Finder_Domain_Repository_AbstractDemandedRepository {
	...
	protected function createCategoryConstraint(Tx_Extbase_Persistence_QueryInterface $query, $categories, $conjunction, $includeSubCategories = FALSE) {
		...
		foreach ($categories as $category) {
			if ($includeSubCategories) {
				$subCategories = t3lib_div::trimExplode(',', Tx_Finder_Service_CategoryService::getChildrenCategories($category, 0, '', TRUE), TRUE);
	 ...
	 *
	 * @param Tx_Extbase_Persistence_QueryInterface $query
	 * @param Tx_Finder_Domain_Model_DemandInterface $demand
	 * @return array<Tx_Extbase_Persistence_QOM_Constraint>
	 */
	protected function createConstraintsFromDemand(Tx_Extbase_Persistence_QueryInterface $query, Tx_Finder_Domain_Model_DemandInterface $demand) {
		$constraints = array();
		...
		if ($demand->getTimeRestriction()) {
			$timeLimit = 0;
			// integer = timestamp
			if (Tx_Finder_Utility_Compatibility::canBeInterpretedAsInteger($demand->getTimeRestriction())) {
		...
		if ($demand->getTimeRestrictionHigh()) {
			$timeLimit = 0;
			// integer = timestamp
			if (Tx_Finder_Utility_Compatibility::canBeInterpretedAsInteger($demand->getTimeRestrictionHigh())) {
		...
		// top finder
		if ($demand->getTopFinderRestriction() == 1) {
			$constraints[] = $query->equals('istopfinder', 1);
		} elseif ($demand->getTopFinderRestriction() == 2) {
			$constraints[] = $query->equals('istopfinder', 0);
		}
		...
		if ($demand->getSearch() !== NULL) {
			/* @var $searchObject Tx_Finder_Domain_Model_Dto_Search */
		...
		if ($demand->getExcludeAlreadyDisplayedFinder() && isset($GLOBALS['EXT']['finder']['alreadyDisplayed']) && !empty($GLOBALS['EXT']['finder']['alreadyDisplayed'])) {
			$constraints[] = $query->logicalNot(
				$query->in(
					'uid',
					$GLOBALS['EXT']['finder']['alreadyDisplayed']
				)
			);
		}
	 ...
	 * @return array<Tx_Extbase_Persistence_QOM_Constraint>
	 */
	protected function createOrderingsFromDemand(Tx_Finder_Domain_Model_DemandInterface $demand) {
		$orderings = array();
		if ($demand->getTopFinderFirst()) {
			$orderings['istopfinder'] = Tx_Extbase_Persistence_QueryInterface::ORDER_DESCENDING;
		}

		if (Tx_Finder_Utility_Validation::isValidOrdering($demand->getOrder(), $demand->getOrderByAllowed())) {
	...
	/**
	 * Find first finder by import and source id
	 *
	...
	/**
	 * Get the count of finder records by month/year and
	 * returns the result compiled as array
	 *
	 * @param Tx_Finder_Domain_Model_DemandInterface $demand
	 * @return array
	 */
	public function countByDate(Tx_Finder_Domain_Model_DemandInterface $demand) {
		$data = array();
		...
		' FROM tx_finder_domain_model_finder ' . substr($sql, strpos($sql, 'WHERE '));
