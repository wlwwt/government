 ...
 */
class Tx_Finder_Domain_Service_CategoryImportService implements t3lib_Singleton {
	...
	protected function hydrateCategory(array $importItem) {
		if (is_null($category = $this->categoryRepository->findOneByImportSourceAndImportId(
			...
			/** @var $category Tx_Finder_Domain_Model_Category */
			$category = $this->objectManager->get('Tx_Finder_Domain_Model_Category');
	...
	protected function createL10nChildrenCategory(array $queueItem) {
		...
		foreach ($titleLanguageOverlay as $key => $title) {
			...
			/** @var $l10nChildrenCategory Tx_Finder_Domain_Model_Category */
