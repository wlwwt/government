 ...
 */
class Tx_Finder_Cache_ClassCacheBuilder {
	...
	public function build() {
		...
		foreach ($extensibleExtensions as $key => $extensionsWithThisClass) {
			...
			// Get the file from finder itself, this needs to be loaded as first
			$path = t3lib_extMgm::extPath('finder') . 'Classes/' . $key . '.php';
			if (!is_file($path)) {
			...
			if ($extendingClassFound) {
				$cacheIdentifier = 'tx_finder_' . strtolower(str_replace('/', '_', $key));
				try {
	...
	/**
	 * Get all loaded extensions which try to extend EXT:finder
	 *
	 * @return array
	 */
	protected function getExtensibleExtensions() {
		$loadedExtensions = array_unique(t3lib_div::trimExplode(',', Tx_Finder_Utility_Compatibility::getLoadedExtensionList(), TRUE));

		// Get the extensions which want to extend finder
		$extensibleExtensions = array();
		foreach ($loadedExtensions as $extensionKey) {
			$extensionInfoFile = t3lib_extMgm::extPath($extensionKey, 'Resources/Private/extend-finder.txt');
	...
	protected function init() {
		...
		require_once(t3lib_extMgm::extPath('finder', 'Classes/Utility/Compatibility.php'));
	}
