 ...
 */
class Tx_Finder_Hooks_SuggestReceiver extends t3lib_TCEforms_Suggest_DefaultReceiver{
...
Ext.Ajax.request({
	url : \'ajax.php\' ,
	params : { ajaxID : \'Finder::createTag\', item:value,finderid:\'' . $uid . '\' },
...
$link = implode(' ', explode(chr(10), $javaScriptCode));
			...
			$records['tx_finder_domain_model_tag_' . strlen($text)] = array (
				'text' => '<div onclick="' . $link . '">
							<span class="suggest-path">
								<a>' .
									sprintf($GLOBALS['LANG']->sL('LLL:EXT:finder/Resources/Private/Language/locallang_be.xml:tag_suggest'), $text) .
								'</a>
							</span></div>',
	...
	private function getDummyIconPath() {
		$icon = t3lib_iconWorks::getIcon('tx_finder_domain_model_tag');
