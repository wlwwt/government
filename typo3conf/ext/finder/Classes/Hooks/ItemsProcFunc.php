 ...
 */
class Tx_Finder_Hooks_ItemsProcFunc {
	...
	public function user_MediaType(array &$config, t3lib_TCEforms $parentObject) {
		...
		if (t3lib_extMgm::isLoaded('dam')) {
			$locallangFile = 'LLL:EXT:finder/Resources/Private/Language/locallang_db.xml:';

				// additional entry
			$damEntry = array(
				$GLOBALS['LANG']->sL($locallangFile . 'tx_finder_domain_model_media.type.I.3'),
				'3',
				t3lib_extMgm::extRelPath('finder') . 'Resources/Public/Icons/media_type_dam.gif'
			);
	...
	public function user_templateLayout(array &$config, t3lib_TCEforms $parentObject) {
		...
		if (isset($GLOBALS['TYPO3_CONF_VARS']['EXT']['finder']['templateLayouts'])
				&& is_array($GLOBALS['TYPO3_CONF_VARS']['EXT']['finder']['templateLayouts'])) {
		...
		if (is_numeric($config['row']['pid'])) {
			...
			if (isset($pagesTsConfig['tx_finder.']['templateLayouts.']) && is_array($pagesTsConfig['tx_finder.']['templateLayouts.'])) {

					// Add every item
				foreach ($pagesTsConfig['tx_finder.']['templateLayouts.'] as $key => $label) {
					$additionalLayout = array(
	 ...
	 * needs different ones then a finder action
	 *
	 * @param array &$config configuration array
	 * @param t3lib_TCEforms $parentObject parent object
	 * @return void
	...
	public function user_orderBy(array &$config, t3lib_TCEforms $parentObject) {
		...
		if (is_array($config['row']) && !empty($config['row']['pi_flexform'])) {
			...
			if (isset($flexformConfig['data']['sDEF']['lDEF']['switchableControllerActions']['vDEF'])) {
				...
				if (t3lib_div::isFirstPartOfStr($selectedActionList, 'Category')) {
					$newItems = $GLOBALS['TYPO3_CONF_VARS']['EXT']['finder']['orderByCategory'];
				} else {
					$newItems = $GLOBALS['TYPO3_CONF_VARS']['EXT']['finder']['orderByFinder'];
				}
			}
		}
		...
		if (!empty($newItems)) {
			...
			$languageKey = 'LLL:EXT:finder/Resources/Private/Language/locallang_be.xml:flexforms_general.orderBy.';
	...
	public function user_switchableControllerActions(array &$config, t3lib_TCEforms $parentObject) {
		if (!empty($GLOBALS['TYPO3_CONF_VARS']['EXT']['finder']['switchableControllerActions']['list'])) {
			$configuration = (int)$GLOBALS['TYPO3_CONF_VARS']['EXT']['finder']['switchableControllerActions']['list'];
				switch ($configuration) {
					case 1:
						$this->removeActionFromList($config, 'Finder->list');
						break;
					case 2:
						$this->removeActionFromList($config, 'Finder->list;Finder->detail');
						break;
					default:
				}
		}
		...
		if (isset($GLOBALS['TYPO3_CONF_VARS']['EXT']['finder']['switchableControllerActions']['newItems'])
				&& is_array($GLOBALS['TYPO3_CONF_VARS']['EXT']['finder']['switchableControllerActions']['newItems'])) {
			foreach ($GLOBALS['TYPO3_CONF_VARS']['EXT']['finder']['switchableControllerActions']['newItems'] as $key => $label) {
	...
	public function user_categoryOverlay(array $config, SC_mod_user_setup_index $parentObject) {
		...
		if (count($languages) > 0) {
			$html = '<select name="data[finderoverlay]" id="field_finderoverlay">
						<option value="0">' . $GLOBALS['LANG']->sL('LLL:EXT:lang/locallang_general.xml:LGL.default_value', TRUE) . '</option>';

			foreach ($languages as $language) {
				$selected = ($GLOBALS['BE_USER']->uc['finderoverlay'] == $language['uid']) ? ' selected="selected" ' : '';
			...
			$html .= $GLOBALS['LANG']->sL(
						'LLL:EXT:finder/Resources/Private/Language/locallang_be.xml:usersettings.no-languages-available', TRUE
					);
		}
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/finder/Classes/Hooks/ItemsProcFunc.php']) {
	require_once ($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/finder/Classes/Hooks/ItemsProcFunc.php']);
}
