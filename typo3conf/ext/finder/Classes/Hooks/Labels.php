...
t3lib_div::requireOnce(t3lib_extMgm::extPath('finder', 'Classes/Domain/Model/Media.php'));
...
class Tx_Finder_Hooks_Labels {
	...
	public function getUserLabelCategory(array &$params) {
		...
		$overlayLanguage = (int)$GLOBALS['BE_USER']->uc['finderoverlay'];
	...
	public function getUserLabelMedia(array &$params) {
		...
		$type = $GLOBALS['LANG']->sL($ll . 'tx_finder_domain_model_media.type.I.' . $params['row']['type']);

			// Add additional info based on type
		switch ((int)$params['row']['type']) {
			// Image
			case Tx_Finder_Domain_Model_Media::MEDIA_TYPE_IMAGE:
				...
				if (!empty($params['row']['image'])) {
					...
					$additionalHtmlContent = '<br />' . t3lib_BEfunc::thumbCode($params['row'], 'tx_finder_domain_model_media', 'image', $GLOBALS['BACK_PATH'], '', NULL, 0, '', '', FALSE);
				}
				break;
				// Audio & Video
			case Tx_Finder_Domain_Model_Media::MEDIA_TYPE_MULTIMEDIA:
				$typeInfo .= $this->getTitleFromFields('caption,multimedia', $params['row']);
				break;
				// HTML
			case Tx_Finder_Domain_Model_Media::MEDIA_TYPE_HTML:
					// Don't show html value as this could get a XSS
				$typeInfo .= $params['row']['caption'];
				break;
				// DAM
			case Tx_Finder_Domain_Model_Media::MEDIA_TYPE_DAM:
		...
		if (is_array($GLOBALS['TYPO3_CONF_VARS']['EXT']['finder']['mediaLabel'])) {
			$params = array('params' => $params, 'title' => $title);
			foreach ($GLOBALS['TYPO3_CONF_VARS']['EXT']['finder']['mediaLabel'] as $reference) {
		...
		if ($params['row']['showinpreview']) {
			$label = htmlspecialchars($GLOBALS['LANG']->sL($ll . 'tx_finder_domain_model_media.show'));
			$icon = '../' . t3lib_extMgm::siteRelPath('finder') . 'Resources/Public/Icons/preview.gif';
	...
	/**
	 * Get finder categories based on the finder id
	 *
	 * @param integer $finderUid
	 * @param integer $catMm
	 * @return string list of categories
	 */
	protected function getCategories($finderUid, $catMm) {
		if ($catMm == 0) {
			return '';
		}
		...
		$res = $GLOBALS['TYPO3_DB']->exec_SELECT_mm_query(
			'tx_finder_domain_model_category.title as title',
			'tx_finder_domain_model_finder',
			'tx_finder_domain_model_finder_category_mm',
			'tx_finder_domain_model_category',
			' AND tx_finder_domain_model_finder.uid=' . (int)$finderUid
		);
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/finder/Classes/Hooks/Labels.php']) {
	require_once ($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/finder/Classes/Hooks/Labels.php']);
}
