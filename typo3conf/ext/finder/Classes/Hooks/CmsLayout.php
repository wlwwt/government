 ...
 */
class Tx_Finder_Hooks_CmsLayout {
	 ...
	 * @var string
	 */
	const LLPATH = 'LLL:EXT:finder/Resources/Private/Language/locallang_be.xml:';
			...
			if (is_array($this->flexformData)) {
				...
				switch ($actionTranslationKey) {
					case 'finder_list':
						...
						$this->getTopFinderRestrictionSetting();
						...
						break;
					case 'finder_detail':
						$this->getSingleFinderSettings();
						$this->getDetailPidSetting();
						break;
					case 'finder_datemenu':
						$this->getStartingPoint();
						$this->getTimeRestrictionSetting();
						$this->getTopFinderRestrictionSetting();
	...
	/**
	 * Render single finder settings
	 *
	 * @return void
	 */
	protected function getSingleFinderSettings() {
		$content = '';
		$singleFinderRecord = (int)$this->getFieldFromFlexform($this->flexformData, 'settings.singleFinder');

		if ($singleFinderRecord > 0) {
			$finderRecord = $GLOBALS['TYPO3_DB']->exec_SELECTgetSingleRow('*', 'tx_finder_domain_model_finder', 'deleted=0 AND uid=' . $singleFinderRecord);

			if (is_array($finderRecord)) {
				$pageRecord = t3lib_BEfunc::getRecord('pages', $finderRecord['pid']);

				if (is_array($pageRecord)) {
					$icon = t3lib_iconWorks::getSpriteIconForRecord('pages', $pageRecord, array('title' => 'Uid: ' . $pageRecord['uid']));
					$onClick = $GLOBALS['SOBE']->doc->wrapClickMenuOnIcon($icon, 'tx_finder_domain_model_finder', $finderRecord['uid'], 1, '', '+info,edit', TRUE);

					$content = '<a href="#" onclick="' . htmlspecialchars($onClick) . '">' . $icon . '</a>' .
						htmlspecialchars(t3lib_BEfunc::getRecordTitle('pages', $pageRecord)) . ': ' .
						htmlspecialchars(t3lib_BEfunc::getRecordTitle('tx_finder_domain_model_finder', $finderRecord)) .
						' <small>(' . $finderRecord['uid'] . ')</small>';
				} else {
					/** @var $message t3lib_FlashMessage */
					$text = sprintf($GLOBALS['LANG']->sL(self::LLPATH . 'pagemodule.pageNotAvailable', TRUE), $finderRecord['pid']);
					$message = t3lib_div::makeInstance('t3lib_FlashMessage', $text, '', t3lib_FlashMessage::WARNING);
					$content = $message->render();
				}
			} else {
				/** @var $message t3lib_FlashMessage */
				$text = sprintf($GLOBALS['LANG']->sL(self::LLPATH . 'pagemodule.finderNotAvailable', TRUE), $singleFinderRecord);
				$message = t3lib_div::makeInstance('t3lib_FlashMessage', $text, '', t3lib_FlashMessage::WARNING);
				$content = $message->render();
			}

			$this->tableData[] = array($GLOBALS['LANG']->sL(self::LLPATH . 'flexforms_general.singleFinder'), $content);
		}
	}
	...
	/**
	 * Render listPid finder settings
	 *
	 * @return void
	 */
	...
	protected function getOrderSettings() {
		...
		if (!empty($orderField)) {
			...
			if (!empty($orderDirection)) {
				...
				// Top finder first
			$topFinderSetting = (int)$this->getFieldFromFlexform($this->flexformData, 'settings.topFinderFirst', 'additional');
			if ($topFinderSetting === 1) {
				$text .= '<br />' . $GLOBALS['LANG']->sL(self::LLPATH . 'flexforms_additional.topFinderFirst');
			}
	...
	protected function getCategorySettings($showCategoryMode = TRUE) {
		...
		if (count($categories) > 0) {
			...
			foreach ($rawCategoryRecords as $record) {
				$categoriesOut[] = htmlspecialchars(t3lib_BEfunc::getRecordTitle('tx_finder_domain_model_category', $record));
			}
	...
	/**
	 * Render top finder restriction configuration
	 *
	 * @return void
	 */
	protected function getTopFinderRestrictionSetting() {
		$topFinderRestriction = (int)$this->getFieldFromFlexform($this->flexformData, 'settings.topFinderRestriction');
		if ($topFinderRestriction > 0) {
			$this->tableData[] = array(
							$GLOBALS['LANG']->sL(self::LLPATH . 'flexforms_general.topFinderRestriction'),
							$GLOBALS['LANG']->sL(self::LLPATH . 'flexforms_general.topFinderRestriction.' . $topFinderRestriction)
						);
		}
	}
	...
	protected function getTemplateLayoutSettings() {
		...
		if (!empty($field) && is_array($GLOBALS['TYPO3_CONF_VARS']['EXT']['finder']['templateLayouts']))
				// Find correct title by looping over all options
			foreach ($GLOBALS['TYPO3_CONF_VARS']['EXT']['finder']['templateLayouts'] as $layouts) {
				if ($layouts[1] === $field) {
					$title = $layouts[0];
				}
			}
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/finder/Classes/Hooks/CmsLayout.php']) {
	require_once ($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/finder/Classes/Hooks/CmsLayout.php']);
}
