...
The template for the RSS feed can be found in the file Resources/Private/Templates/Finder/List.xml.
The "magic" which uses the List.xml template instead of the List.html is the following configuration: ::

   plugin.tx_finder.settings.format = xml
...
A very simple way to generate the RSS feed is using plain TypoScript. All you need is to use the given TypoScript and adopt it to your needs. ::
    ...
    pageFinderRSS {
    	typeNum = 9818
    	10 < tt_content.list.20.finder_pi1
    	10 {
...
This example will show all finder records which don't have the category with the uid 9 assigned and are saved on the page with uid 24. The single view page is the one with uid 25.
...
#. Add the finder plugin and define the configuration you need. E.g. startingpoint, page with the single view, ...
...
The TypoScript code looks like this. ::
    ...
    [globalVar = TSFE:type = 9818]
    	...
    	pageFinderRSS = PAGE
    	pageFinderRSS.typeNum = 9818
    	pageFinderRSS.10 < styles.content.get
    	pageFinderRSS.10.select.where = colPos=0 AND list_type = "finder_pi1"
    	pageFinderRSS.10.select {
    		orderBy = sorting ASC
    		max = 1
    	}
...
**Some explanations**
The page object pageFinderRSS will render only those content elements which are in colPos 0 and are a finder plugin. Therefore all other content elements won't be rendered in the RSS feed.
