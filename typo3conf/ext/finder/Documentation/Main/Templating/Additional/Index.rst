...
Using the following Page TsConfig the editor can select the layouts in the finder plugin: ::

	tx_finder.templateLayouts {
		1 = A custom layout
	...
	<f:if condition="{finder}">
		<f:then>
			<f:if condition="{settings.templateLayout} == 99">
				<f:then>
					<div class="finder well finder-special">
						<f:for each="{finder}" as="finderItem">
							<f:render partial="List/Item-special" arguments="{finderItem: finderItem, settings:settings}"/>
						</f:for>
					</div>
				</f:then>
				<f:else>
					<div class="finder finder-list-view">
						<n:widget.paginate objects="{finder}" as="paginatedFinder" configuration="{settings.list.paginate}">
								<f:for each="{paginatedFinder}" as="finderItem">
									<f:render partial="List/Item" arguments="{finderItem: finderItem, settings:settings}"/>
								</f:for>
						</n:widget.paginate>
					</div>
				</f:else>
		...
		<f:else>
			<div class="no-finder-found"><f:translate key="list_nofinderfound" /></div>
		</f:else>
	</f:if>
...
Sometimes it is useful to iterate over finder items and use different classes for every finder item.
This examples shows how you can cycle through 3 css classes and repeat those. ::

	<n:widget.paginate objects="{finder}" as="paginatedFinder" configuration="{settings.list.paginate}">
		<f:for each="{paginatedFinder}" as="finderItem" iteration="iterator">
			<f:cycle values="{0: 'left', 1: 'center', 2: 'right'}" as="cycle">
				<li class="{cycle}">
					<strong>position: {cycle} // finderUID: {finderItem.uid}</strong>
...
If you want to create a condition based on a spefic category, you can use something like this: ::
	...
	<f:for each="{finderItem.categories}" as="category">
...
If you want to customize the Paginate ViewHelper Template you can customize it by following this example: ::
	...
	plugin.tx_finder {
		view {
			widget.Tx_Finder_ViewHelpers_Widget_PaginateViewHelper.templateRootPath = fileadmin/pathtocustomfinder/Templates/
		}
	}

The template needs to be then saved in this place: ::

	fileadmin/pathtocustomfinder/Templates/ViewHelpers/Widget/Paginate/Index.html
